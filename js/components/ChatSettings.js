...
export default {
    ...
    setup(props, { emit }) {
        ...
        if (!localSettings.value.settings.fontSize) localSettings.value.settings.fontSize = 13;
        if (!localSettings.value.settings.activeStickerIds) localSettings.value.settings.activeStickerIds = [];
        if (localSettings.value.settings.enableTranslation === undefined) localSettings.value.settings.enableTranslation = false;
        ...
        const isActiveSticker = (item) => { return localSettings.value.settings.activeStickerIds.includes(item.url); };
        ...
        const toggleSelectAll = () => { if (isAllSelected.value) selectedItems.value = []; else selectedItems.value = [...currentFolderList.value]; };
        const getAllImageUrlsInItem = (item) => { if (item.type === 'image') return [item.url]; if (item.type === 'folder' && item.children) return item.children.flatMap(getAllImageUrlsInItem); return []; };
        const triggerBatchActive = (enable) => {
            ...
            selectedItems.value.forEach(item => {
                ...
                urls.forEach(url => {
                    ...
                    if (enable) { if (idx === -1) activeList.push(url); } else { if (idx > -1) activeList.splice(idx, 1); }
                });
            });
            cancelMultiSelect();
        };
        ...
        onMounted(() => {
            ...
            allItems.filter(i => i.type === 'book').forEach(book => { if (book.parentId && folderMap[book.parentId]) folderMap[book.parentId].children.push(book); else unclassified.children.push(book); });
            let result = []; Object.values(folderMap).forEach(f => result.push(f)); if (unclassified.children.length > 0) result.push(unclassified); fullTree.value = result.filter(n => n.children.length > 0);
        });
        const filteredTree = computed(() => { if (!searchQuery.value.trim()) return fullTree.value; const query = searchQuery.value.toLowerCase(); return fullTree.value.map(folder => { if (folder.title.toLowerCase().includes(query)) return folder; const matchingChildren = folder.children.filter(book => book.title.toLowerCase().includes(query)); if (matchingChildren.length > 0) return { ...folder, children: matchingChildren, isOpen: true }; return null; }).filter(Boolean); });
        ...
        const toggleFolder = (folder) => { const getAllBookIds = (n) => { if (n.type === 'book') return [n.id]; return n.children.flatMap(getAllBookIds); }; const childIds = getAllBookIds(folder); const allSelected = childIds.every(id => activeIds.value.includes(id)); if (allSelected) activeIds.value = activeIds.value.filter(id => !childIds.includes(id)); else childIds.forEach(id => { if (!activeIds.value.includes(id)) activeIds.value.push(id); }); };
        ...
        const handleFile = (event, type) => { const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = (e) => { if (type === 'aiAvatar') localSettings.value.avatar = e.target.result; else if (type === 'userAvatar') localSettings.value.settings.userAvatar = e.target.result; else localSettings.value.settings.background = e.target.result; }; reader.readAsDataURL(file); };
        ...
        return { 
            ...
            isAppearanceOpen, 
            openAppearanceSettings, cancelAppearance, saveAppearance, sliderStyleDynamic,
            handleImgError
        };
    }
};
